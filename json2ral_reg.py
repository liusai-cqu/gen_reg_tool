...
def generate_ral_model(module_name, registers):
    ...
    """
    ral_model_code = f"""
class {module_name}_reg_model extends uvm_reg_block;

    `uvm_object_utils({module_name}_reg_model)
...
"""
    ...
    for register in registers:
        reg_name = register["REG_NAME"]
        ral_model_code += f"    uvm_reg {reg_name};\n"

    ral_model_code += f"""

    function new (string name = "{module_name}_reg_model");
...
"""
    ...
    for register in registers:
        ...
        ral_model_code += f"""
        {reg_name} = {reg_name}::type_id::create("{reg_name}",,get_full_name());
        {reg_name}.configure(this, null, "{reg_name}");
        {reg_name}.build();
        {reg_name}.add_map("apb_map", '{reg_address}, 1, `UVM_LITTLE_ENDIAN, 1);
"""
...
class {reg_name} extends uvm_reg;
    ...
    rand uvm_reg_field {", rand uvm_reg_field ".join([field["NAME"] for field in fields])};
...
def generate_field_configuration(field, reg_name):
    ...
    return f"""
        ...
        this.{field_name}.configure(this, {field_width}, {field_lsb}, "{field_access}", 0, {field_reset}, 1, 1);
"""
